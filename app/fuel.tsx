...
function useFuelTelemetry() {
    ...
    const fetchLatest = async () => {
        try {
            ...
            setData({
                systemStatus: row?.fuel_system_status ?? 'N/A',
                pressureKpa: row?.fuel_pressure_kpa != null ? String(row.fuel_pressure_kpa) : 'N/A',
                pressurePsi: row?.fuel_pressure_psi != null ? String(row.fuel_pressure_psi) : 'N/A',
                levelPercent: row?.fuel_level_pct != null ? String(row.fuel_level_pct) : 'N/A',
                fuelType: row?.fuel_type ?? 'N/A',
...
export default function FuelScreen() {
    const { data, refresh, loading } = useFuelTelemetry();
	...
	return (
		<SafeAreaView style={styles.safeArea}>
			...
			>
				...
				<Text style={styles.subtitle}>Monitor your vehicle&apos;s fuel system</Text>
				...
				<View style={styles.card}>
					<View style={styles.metricRow}>
						<MetricTile iconName="analytics-outline" iconBg="#EAF6EC" iconColor="#2ecc71" label="Fuel System Status" value={data.systemStatus} />
						<MetricTile iconName="water-outline" iconBg="#E9F5FF" iconColor={accent} label="Fuel Pressure (kPa)" value={`${data.pressureKpa} kPa`} />
					</View>
					<View style={styles.metricRow}>
						<MetricTile iconName="water" iconBg="#E9F5FF" iconColor={accent} label="Fuel Pressure (PSI)" value={`${data.pressurePsi} PSI`} />
						<MetricTile iconName="beaker-outline" iconBg="#FFF4DD" iconColor="#f1c40f" label="Fuel Level" value={`${data.levelPercent}%`} />
					</View>
					<View style={styles.metricRow}>
						<MetricTile iconName="flame" iconBg="#FFEFF2" iconColor="#e74c3c" label="Fuel Type" value={data.fuelType} />
						<MetricTile iconName="trending-up" iconBg="#EAF6EC" iconColor="#2ecc71" label="STFT" value={`${data.stft}%`} />
					</View>
					<View style={styles.metricRow}>
						<MetricTile iconName="trending-down" iconBg="#FFF4DD" iconColor="#f1c40f" label="LTFT" value={`${data.ltft}%`} />
					</View>
				</View>
